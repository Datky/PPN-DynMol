...
bool checkifupdate(const Particules& p, f64 update){
	for(u32 i = 0;i<N;++i){
		f64 dx = abs(p.pos->X[i] - p.prev_pos->X[i]);
		f64 dy = abs(p.pos->Y[i] - p.prev_pos->Y[i]);
		f64 dz = abs(p.pos->Z[i] - p.prev_pos->Z[i]);
		f64 dist_moved = sqrt(dx*dx+dy*dy+dz*dz);

		if(dist_moved >= update){
			return true;
		}
	}
	return false;
