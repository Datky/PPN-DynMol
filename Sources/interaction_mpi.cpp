                  ...
                  for (int k = 0; k < c_x; ++k) {
                        ...
                        for (u32 ind = 0; ind < vec[i+1][j+1][k+1].size(); ++ind) {
                              ...
                              */
                              ...
                              while (part.pos.X[vrai_id] < 0) {part.pos.X[vrai_id] = b_x+part.pos.X[vrai_id];}
                              while (part.pos.X[vrai_id] > b_x) {part.pos.X[vrai_id] = part.pos.X[vrai_id]-b_x;} 

                              while (part.pos.Y[vrai_id] < 0) {part.pos.Y[vrai_id] = b_y+part.pos.Y[vrai_id];}
                              while (part.pos.Y[vrai_id] > b_y) {part.pos.Y[vrai_id] = part.pos.Y[vrai_id]-b_y;} 
                               
                              while (part.pos.Z[vrai_id] < 0) {part.pos.Z[vrai_id] = b_z+part.pos.Z[vrai_id];}
                              while (part.pos.Z[vrai_id] > b_z) {part.pos.Z[vrai_id] = part.pos.Z[vrai_id]-b_z;} 
