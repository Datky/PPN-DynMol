...
void remplissage_vecteurs(struct Particule* liste) { // Remplit les vecteurs de données
    ...
    generation_gaussienne_des_vitesses(liste);
    std::cout << "\nBonne affectation vitesses selon une fonction gaussienne.\n" << std::endl;
    
    accelerations_initiales_nulles(liste);
    std::cout << "\nBonne affectation des accélérations avec des valeurs nulles.\n" << std::endl;
}

void generation_gaussienne_des_vitesses(struct Particule* liste){
...
// Calcul de la composante selon x, y et z du vecteur quantité de mouvement total du système.
    for(u64 i = 0; i < N; i++){
        liste[i].vit_X = d_v_x(gen);
        qdm_systeme_x = qdm_systeme_x + m*(liste[i].vit_X);
        liste[i].vit_Y = d_v_y(gen);
        qdm_systeme_y = qdm_systeme_y + m*(liste[i].vit_Y);
        liste[i].vit_Z = d_v_z(gen);
        qdm_systeme_z = qdm_systeme_z + m*(liste[i].vit_Z);
    }
...
// Ajustement de qdm_systeme à 0, ie m*vit->X[i] = m*vit->X[i] - qdm_systeme_x, ETC.
    for(u64 i = 0; i < N; i++){
        liste[i].vit_X -= qdm_systeme_x/N/m;
        liste[i].vit_Y -= qdm_systeme_y/N/m;
        liste[i].vit_Z -= qdm_systeme_z/N/m;
...
// Qdm totale après ajustement
    ...
    for(u64 i = 0; i < N; i++){
        qdm_systeme_après_x += m*(liste[i].vit_X);
        qdm_systeme_après_y += m*(liste[i].vit_Y);
        qdm_systeme_après_z += m*(liste[i].vit_Z);
    }
...
// Calcul de la température en fonction de la norme du vecteur vitesse.
    f64 temperature = 0;
    for(u64 i = 0; i < N; i++){
        temperature = temperature + 1/(3*k_b*N)*m*(pow(liste[i].vit_X,2.0)+pow(liste[i].vit_Y,2.0)+pow(liste[i].vit_Z,2.0));
    }

// Calcul de la vitesse selon initialisation à temperature_cible;
    for(u64 i = 0; i < N; i++){
        liste[i].vit_X = (liste[i].vit_X)*sqrt(temperature_cible/temperature);
//        std::cout << &vit->X[i] << ": vit_Gauss_après_ajustement_Tcible_X = " <<vit->X[i] << std::endl;
        liste[i].vit_Y = (liste[i].vit_Y)*sqrt(temperature_cible/temperature);
//        std::cout << &vit->Y[i] << ": vit_Gauss_après_ajustement_Tcible_Y = " <<vit->Y[i] << std::endl;
        liste[i].vit_Z = (liste[i].vit_Z)*sqrt(temperature_cible/temperature);
//        std::cout << &vit->Z[i] << ": vit_Gauss_après_ajustement_Tcible_Z = " <<vit->Z[i] << std::endl;
    }
}

void accelerations_initiales_nulles(struct Particule* liste){
    for(u64 i = 0; i < N; i++){
        liste[i].acc_X = 0.0;
