...
int main(int argc, char **argv) {
    ...
    for (u64 i = 1; i <= nb_iteration; i++) {
        ...
        if (i<10)
        {
            ...
            std::cout << "[0" << i << "/" << nb_iteration << "] : Bonne création du fichier .xyz de la 0" << i << "-ème itération et écriture des positions (version OpenMP)." << std::endl;
        } else {
            ecrireXYZ(positions, "Sortie_omp/simulation"+str_N+"_iteration"+fichier_i+".xyz");
            std::cout << "[" << i << "/" << nb_iteration << "] : Bonne création du fichier .xyz de la " << i << "-ème itération et écriture des positions (version OpenMP)." << std::endl;
        }
    }
