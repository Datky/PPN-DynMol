...
void writeDataToFile(int N, long long total){
        std::ofstream outputFile("data.txt",std::ios::app);
        if(outputFile.is_open()){
                outputFile << N << " " << total <<std::endl;
                outputFile.close();
        }
        else{
...
int main(int argc, char **argv) {
    ...
    f64 r_cut = 3.0*d; //voisin截断半径，单位：ai;
    f64 skin_thickness = 0.5 * r_cut; 
    std::vector<u32> NN(N,0);
    std::vector<u32> NL(N*MN,0);
    f64 max_displacement = 0.0;
    ...
    for (u64 i = 1; i <= nb_iteration; i++) {
       ...
       for (size_t step = 0; step < nb_iteration; ++step) {
    // 检查最大位移是否大于等于皮肤厚度的一半
    if (max_displacement >= 0.5 * skin_thickness) {
        // 重新构建 Verlet 列表
        Voisin(particules, r_cut, b_x, b_y, b_z, NN, NL);
        // 重置 max_displacement
        max_displacement = 0.0;
    }
	   
	    Voisin(particules, r_cut,skin_thickness, b_x, b_y, b_z, NN, NL);
        VerletCellules(vec, particules, r_cut_carre, frontiere_type, NN, NL);
        majPositionsetCellules(vec, particules, r_cut_carre, frontiere_type, max_displacement);
